   ...
   $stmt =$conn->prepare($query);
		...
		$details= $result->fetch_assoc();
			if($result->num_rows >0){
				$sql="UPDATE scale_adviser_report_tbl set s_a_endorsement_remarks=? WHERE student_u_id = ? AND adviser_u_id=?";
				...
				$sql = "INSERT INTO scale_adviser_report_tbl (s_a_endorsement_remarks_remarks,student_u_id, adviser_u_id) VALUES (?, ?, ?)";
...
<script>
  ...
  location.replace("view_scad_endorsement_form.php")
